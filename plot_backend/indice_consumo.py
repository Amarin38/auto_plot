...
class IndiceConsumo:
    ...
    def calcular_indice_por_coche(self) -> List[Union[pd.DataFrame, str]]:
        ...
        df_consumo = pd.read_excel(f"{self._main_path}/excel/{self.file_consumo}-S.xlsx", engine="calamine")

        repuestos: ndarray = df_consumo["Repuesto"].unique() 
        cabeceras: ndarray = df_consumo["Cabecera"].unique()
        ...
        for cab in cabeceras:
            ...
            for rep in repuestos:
                total_consumo_por_cabecera = df_consumo.loc[(df_consumo["Repuesto"] == rep) & 
                                                                 (df_consumo["Cabecera"] == cab),
                                                                 ['Cantidad']].sum()
                
                total_coste_por_cabecera = df_consumo.loc[(df_consumo["Repuesto"] == rep) &
                                                               (df_consumo["Cabecera"] == cab), 
                                                               ['Precio']].sum()
        ...
        return [df_indice, self.fecha_titulo(df_consumo)]
    ...
    def calcular_indice_por_motores(self) -> List[Union[pd.DataFrame, str]]:
        ...
        for cab in cabeceras:
            cab_consumo: pd.Series[bool] = df_consumo["Cabecera"] == cab 
            cab_motores: pd.Series[bool] = self.df_motores["Cabecera"] == cab 

            for rep in repuestos:
                indice_consumo: float = 0

                rep_consumo: pd.Series[bool] = df_consumo["Repuesto"] == rep
