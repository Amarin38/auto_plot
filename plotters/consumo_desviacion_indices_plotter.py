...
class DeviationPlotter:
    ...
    def create_plot(self) -> go.Figure:
        if self.tipo_rep:
            self.df["DesviacionRepuesto"] = pd.to_numeric(self.df["DesviacionRepuesto"]).round(2)

            x_data = self.df["Cabecera"]
            y_data = self.df["DesviacionRepuesto"]
        else:
            agrupado = self.df.groupby("Cabecera").agg({"DesviacionCabecera": "mean"}).reset_index()
            agrupado["DesviacionCabecera"] = pd.to_numeric(agrupado["DesviacionCabecera"]).round(2)
            
            x_data = agrupado["Cabecera"]
            y_data = agrupado["DesviacionCabecera"]
