...
class IndexPlotter:
    ...
    def create_plot(self) -> tuple[list[Any], str] | tuple[None, None]:
        ...
        self.df["IndiceConsumo"] = self.df["IndiceConsumo"].astype(float)
        ...
        for repuesto, color in zip(todos_repuestos, IndiceColorsEnum.as_list()):
            ...
            condicion_mayor = df_repuesto["IndiceConsumo"] > avg
            condicion_menor = df_repuesto["IndiceConsumo"] < avg
            condicion_igual = df_repuesto["IndiceConsumo"] == avg

            porcentaje_mayor = round((df_repuesto.loc[condicion_mayor, "IndiceConsumo"] * 100) / avg, 0) - 100
            porcentaje_menor = round((df_repuesto.loc[condicion_menor, "IndiceConsumo"] * 100) / avg, 0) - 100
