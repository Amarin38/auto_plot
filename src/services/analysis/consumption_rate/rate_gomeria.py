...
class RateGomeria:
    def __init__(self, file: str, diff_months: int) -> None:
        ...
        self.df = pd.read_excel(f"{self.file}.xlsx", engine="calamine")
        self.data_consumo = pd.read_excel(f"{MAIN_PATH}/src/data/excel_data/cubiertas_consumo_{self.file}.xlsx", engine="calamine")
    ...
    def update_tire_values(self) -> Dict[str, float]:
        meses: pd.Index = RateUtils()._create_months_list(self.diff_months)
