...
from src.services.analysis.forecast.utils.forecast_utils import ForecastUtils
...
class ForecastTrend:
    ...
    def calculate_trend(self, df: pd.DataFrame) -> pd.DataFrame:
        meses_en_adelante = ForecastUtils._calculate_date_trend(self.meses_en_adelante)
    ...
    def calcular_v2(self, df:pd.DataFrame) -> pd.DataFrame:
        ...
        y_consumo = df.groupby(["Repuesto", "TotalMes"]) 
        x_indices = len(y_consumo)

        indice_mes = np.arange(x_indices, x_indices + self.meses_en_adelante)

        poly = Polynomial.fit(x_indices, y_consumo, deg=1).convert() # calculo tendencia lineal # type: ignore
    ...
    def calculate_seasonal_rate(self, df: pd.DataFrame, df_tendencia: pd.DataFrame) -> List[float]:
