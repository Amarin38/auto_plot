...
class InventoryDelete:
    def __init__(self, file: Union[str, pd.DataFrame]) -> None:
        self.df: pd.DataFrame = CommonUtils()._convert_to_df(file) # type: ignore

    @execute_safely
    def _delete_unnamed_cols(self) -> pd.DataFrame:
        """ Deletes all the 'Unnamed' columns. """
        self.df = self.df.loc[:, ~self.df.columns.str.contains("Unnamed")] 
        self.df = self.df.loc[:, ~self.df.columns.str.contains("Columna")]

        # self.df.to_excel(f"{OUT_PATH}/{self.file}.xlsx")
        return self.df

    @execute_safely
    def _delete_rows(self, delete_type: str, delete_by: Tuple[str, ...]) -> pd.DataFrame:
        """
