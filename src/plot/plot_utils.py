...
class PlotUtils:
    @staticmethod
    def _auto_annotate_on_line(x_data: List[str], y_data: List[str], axs, color: str) -> None:
        """
        ...
        for y, x in zip(y_data, x_data): # labels tendencia
                    axs.annotate(
                        ...
                        ha='center',
                        va='bottom',
                        fontsize=15/PlotConfig.DIV_FACTOR,
                        color=color
                    )
    ...
    def _auto_median_line(ax, median_data: float, additional_text = "") -> None:
        ...
        ax.axhline(y=median_data, linestyle="--", color=COLORS[3])
        ax.text(x=1.01, 
                y=median_data, 
