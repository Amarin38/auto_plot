...
from presentation.streamlit_components import SelectBoxComponents, GoogleSheetsComponents, Paginate
...
def maximos_minimos():
    ...
    if cabecera_seleccionada:
        ...
        df_sheet = get_sheet()
        df_sheet = df_sheet.astype({
            "Descripcion": str,
            "Cabecera": "category",
            "Familia": "UInt16",
            "Articulo": "UInt32",
            "DescripcionStock": str,
            "CabeceraStock": "category",
            "FamiliaStock": "UInt16",
            "ArticuloStock": "UInt32",
        })
        ...
        df_max_min_filtrado["Familia"] = pd.to_numeric(df_max_min_filtrado["Familia"], errors='coerce')
        df_max_min_filtrado["Articulo"] = pd.to_numeric(df_max_min_filtrado["Articulo"], errors='coerce')
        df_max_min_filtrado["Descripcion"] = df_max_min_filtrado["Descripcion"].fillna("")
        ...
        if filtros.familia:
            mask_max_min &= (
                    df_max_min_filtrado["Familia"] == int(str(filtros.familia).strip())
            ).fillna(False)

        if filtros.articulo:
            mask_max_min &= (
                    df_max_min_filtrado["Articulo"] == int(str(filtros.articulo).strip())
            ).fillna(False)

        if filtros.descripcion:
            mask_max_min &= (
                df_max_min_filtrado["Descripcion"]
                    .str.upper()
        ...
        paginate.update_filters(filtros, "maxmin", MAX_MIN_PAGER_KEY)
        ...
        df_stock_filtrado["FamiliaStock"] = pd.to_numeric(df_stock_filtrado["FamiliaStock"], errors='coerce')
        df_stock_filtrado["ArticuloStock"] = pd.to_numeric(df_stock_filtrado["ArticuloStock"], errors='coerce')
        df_stock_filtrado["DescripcionStock"] = df_stock_filtrado["DescripcionStock"].fillna("")
        ...
        if filtros.familia:
            mask_max_min_stock &= (
                    df_stock_filtrado["FamiliaStock"] == int(str(filtros.familia).strip())
            ).fillna(False)

        if filtros.articulo:
            mask_max_min_stock &= (
                    df_stock_filtrado["ArticuloStock"] == int(str(filtros.articulo).strip())
            ).fillna(False)

        if filtros.descripcion:
            mask_max_min_stock &= (
                df_stock_filtrado["DescripcionStock"]
                    .str.upper()
                    .str.startswith(str(filtros.descripcion.strip().upper()), na=False)
            )
        ...
        if pestaña_activa == "↕️ Máximos y Mínimos":
            if len(df_max_min):
                df_paginado, paginas = paginate.create_pagination(df_max_min_filtrado, 15, MAX_MIN_PAGER_KEY, False)
                ...
                paginate.create_buttons(paginas, key=MAX_MIN_PAGER_KEY)
                ...
                def guardar_calculos_automaticos(df_stock_actualizado):
                    with st.spinner("Recalculando Máximos y Mínimos..."):
                        with st.spinner(f"Recalculando y actualizando {cabecera_seleccionada}..."):
                            ...
                            df_para_guardar.loc[:, "Fecha"] = pd.to_datetime(df_para_guardar["Fecha"], errors="coerce", format=FILE_STRFTIME_YMD)
