    ...
    with st.container(height=DURACION_TAB_BOX_HEIGHT):
        if select_rep:
            ...
            with datos.container(height=155):
                st.write(f"{HTML_SIN_CAMBIOS} Sin cambios en 2015: {duracion.calcular_sin_cambios("2015")}</p>",
                            unsafe_allow_html=True)
                st.write(f"{HTML_SIN_CAMBIOS} Sin cambios en 2016: {duracion.calcular_sin_cambios("2016")}</p>",
                            unsafe_allow_html=True)
                st.write(f"{HTML_SIN_CAMBIOS} Sin cambios en 2017: {duracion.calcular_sin_cambios("2017")}</p>",
                            unsafe_allow_html=True)
